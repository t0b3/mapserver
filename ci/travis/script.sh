...
if [ "$BUILD_NAME" = "PHP_7.2_WITH_ASAN" ]; then
    ...
    make cmakebuild MFLAGS="-j2" CMAKE_C_FLAGS="-O2" CMAKE_CXX_FLAGS="-O2" PYTHON_EXECUTABLE="python"
    make mspython-wheel
    make -j4 test
else
    # Force use of PROJ 4 API
    sudo rm /usr/include/proj.h
    make cmakebuild MFLAGS="-j2" CMAKE_C_FLAGS="-DACCEPT_USE_OF_DEPRECATED_PROJ_API_H" CMAKE_CXX_FLAGS="-DACCEPT_USE_OF_DEPRECATED_PROJ_API_H" LIBMAPSERVER_EXTRA_FLAGS="-Wall -Werror" PYTHON_EXECUTABLE="python3"
    make mspython-wheel
    make -j4 test
fi
